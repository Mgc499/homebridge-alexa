...
function endPoints(message, accessories, filter, speakers) {
  ...
  accessories.forEach(function(element) {
    devices.push(_parseHAPtoAlexa(element, filter, speakers));
  });
  var response = {
    "event": {
      "header": {
...
"host": host,
"port": port,
"HBname": hapname, - Hostname
"hbUsername": hbUsername, - homebridge username
"accessories": body - This is HAP Accessory object

 */

function _parseHAPtoAlexa(element, filter, speakers) {
  var alexaDevices = {};
  let acookie = {};
  let ycookie = {};

  for (var accessory in element.accessories) {
    let aid = parseInt(element.accessories[accessory].aid);
    let device = element.accessories[accessory];
    ...
    for (var service in device.services) {
      ...
      if (serviceType.startsWith("00000043") || serviceType.startsWith("00000047") ||
        ...
        for (var id in device.services[service].characteristics) {
          ...
          description = _fixHapName(element.HBname) + " " + name + " " + _lookupFriendlyDisplayCategory(serviceType);
          ...
          switch (type.substring(0, 8)) {
            ...
            case "00000025": // Accessory On/Off
              ...
              if (isAppleTV()) {
                ...
                if (_atvNameTranslate(name) === "Sleep") {
                  cookie = acookie;
                  acookie = {};
                  ...
                  description = element.HBname + " " + name + " " + "Apple TV";
                ...
                if (_yamahaTranslate(name) === "Rewind") {
                  ...
                  // Overide defaults for Apple TV Device
                  name = "Stereo";
                  description = element.HBname + " " + name + " " + "Yamaha Playback";
            ...
            case "00000008": // Brightness
              ...
              if (isSpeaker()) { // Alexa.Speaker and Alexa.StepSpeaker
                ...
                displayCategories = ["SPEAKER"];
                description = _fixHapName(element.HBname) + " " + name + " Speaker";
        ...
        if (capabilities.length > 1) {
          ...
          if ((filter && filter === element.host + ":" + element.port) || !filter) {
            alexaDevices[element.host + ":" +
              element.port + ",aid: " + aid.toString() + ",iid: " + iid.toString()] = {
              'friendlyName': name,
              'description': description,
              'modelName': model,
              'manufacturerName': manufacturer,
              'endpointId': manufacturer + model + element.hbUsername + name,
              'cookie': cookie,
  ...
  debug("Alexa Controllable", element.HBname, Object.keys(alexaDevices).length);
  return alexaDevices;
}
