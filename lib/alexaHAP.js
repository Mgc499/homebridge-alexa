...
function _discovery() {
  ...
  try {
    browser = bonjour.find({
      ...
      //debug('Found an HAP server:', service.txt.id);
      ...
      debug("HAP instance address: %s -> %s -> %s", service.name, service.txt.id, hostname);
      _getAccessories(hostname, service.port, service.name, service.txt.id, function(err, data) {
        if (!err) {
...
function _getAccessories(host, port, hapname, hbUsername, callback) {
  request({
    method: 'GET',
